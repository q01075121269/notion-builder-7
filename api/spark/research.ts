// api/spark/research.ts
// Vercel Serverless & 로컬 Vite 프록시 핸들러 (Gemini Google Search Grounding)
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!topic) {
      res.statusCode = 400;
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({ error: '리서치 주제(topic)가 누락되었습니다.' }));
      return;
    }
...
[필수 반환 JSON 데이터 스키마]
{
  ...
  "sources": [
    ...
    if (!response.ok) {
      ...
      console.error('[Gemini Google Search Grounding Error in Serverless]', response.status, errText);
      res.statusCode = response.status;
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({ error: `Gemini API 호출 실패 (${response.status}): ${errText}` }));
      return;
    }
    ...
    res.statusCode = 200;
    res.setHeader('Content-Type', 'application/json');
    res.end(JSON.stringify(parsedPayload));
  } catch (error: any) {
    console.error('[Spark Research API Handler Exception]', error);
    res.statusCode = 500;
    res.setHeader('Content-Type', 'application/json');
    res.end(JSON.stringify({ error: error.message || '자율 웹 리서치 처리 중 예외가 발생했습니다.' }));
  }
}
