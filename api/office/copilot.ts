// api/office/copilot.ts
// Vercel Serverless & 로컬 프록시 핸들러
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!prompt) {
      res.statusCode = 400;
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({ error: '지시 내용(prompt)이 누락되었습니다.' }));
      return;
    }
...
[작성 및 변이 규칙]
1. 사용자가 요구한 주제(예: '스마트 시설 유지 관리', 'AI 에이전트 도입' 등)에 정확히 부합하는 실무급 행정 기안서 체계로 전문성 있게 작성할 것.
...
위 내용을 기반으로 공문서 기안 JSON을 생성해 주십시오.`;
    ...
    for (const m of candidateModels) {
      try {
        ...
        if (!gRes.ok) {
          ...
          lastError = errData?.error?.message || `모델 ${m} 응답 코드: ${gRes.status}`;
          continue;
        }
    ...
    res.statusCode = 502;
    res.setHeader('Content-Type', 'application/json');
    res.end(JSON.stringify({ error: `Gemini 기안문 생성 실패: ${lastError || '알 수 없는 오류'}` }));
  } catch (err: any) {
    res.statusCode = 500;
    res.setHeader('Content-Type', 'application/json');
