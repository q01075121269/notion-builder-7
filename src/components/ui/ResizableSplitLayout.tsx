...
export const ResizableSplitLayout: React.FC<ResizableSplitLayoutProps> = ({
  ...
  const toggleCollapse = useCallback(() => {
    if (isCollapsed || ratio === 0) {
      const restoreWidth = lastExpandedRatio > minRatio ? lastExpandedRatio : defaultRatio;
  ...
  }, [isCollapsed, ratio, lastExpandedRatio, minRatio, defaultRatio]);

  // 마우스/터치 리사이징 로직 (Pointer Events + requestAnimationFrame)
  const handlePointerDown = useCallback((e: React.PointerEvent) => {
    ...
    const onPointerMove = (moveEvent: PointerEvent) => {
      ...
      animationFrameRef.current = requestAnimationFrame(() => {
        ...
        const snapThreshold = pixelThreshold * 0.75; // 75% 지점에서 스냅
  ...
  return (
    <div
      ref={containerRef}
      className={`relative flex flex-col md:flex-row w-full h-full overflow-hidden select-none ${className}`}
    >
      ...
      {/* 좌측 패널 (기본 25% / 모바일은 상하 스택) */}
      <div
        className={`w-full transition-[width] ease-out shrink-0 overflow-hidden ${
      ...
      <div
        ...
        title="마우스로 드래그하여 패널 너비를 조절하거나 알약 버튼을 클릭하여 접으세요"
      >
        {/* 세로 구분선 가이드라인 */}
        <div
        ...
        <button
          ...
          title={isCollapsed ? '좌측 사이드바 펼치기' : '좌측 사이드바 접기 (전체화면 모드)'}
        >
          {isCollapsed ? (
