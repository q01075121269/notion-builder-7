...
export const TemplatePreviewCanvas: React.FC<TemplatePreviewCanvasProps> = ({ template }) => {
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // [좌측 영역 (기본 25%)]: 네비게이션 바로가기, 에이전트 상태 모니터, 추천 프리셋
  // ───────────────────────────────────────────────────────────────────────────
  const leftSidebarContent = (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900/60 border-r border-slate-200 dark:border-slate-800 p-3.5 space-y-4 text-slate-800 dark:text-slate-200">
      ...
      {/* 3. 추천 에이전트 스킬 프리셋 (Agent Skills Presets) */}
      <div className="rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-3 shadow-xs">
        <div className="flex items-center space-x-1.5 mb-2">
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // [우측 메인 영역 (기본 75%)]: 상단 KPI 바 + 캔버스 본문
  // ───────────────────────────────────────────────────────────────────────────
  const rightMainContent = (
    <div className="flex flex-col min-h-full bg-white dark:bg-notion-dark-bg">
      {/* 1. 상단 모노톤 KPI 메트릭 바 */}
      <div className="sticky top-0 z-10 px-4 sm:px-8 py-2.5 bg-slate-50/95 dark:bg-slate-900/90 backdrop-blur-xs border-b border-slate-200/90 dark:border-slate-800 flex items-center justify-between gap-3 overflow-x-auto select-none shadow-2xs">
  ...
  return (
    <div className="w-full h-full overflow-hidden">
      <ResizableSplitLayout
        ...
        maxRatio={45}
        minPixelWidth={230}
        storageKey="template_builder_preview_split"
        className="h-full"
      />
    </div>
  );
