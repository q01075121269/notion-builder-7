    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        if (req.url && req.url.startsWith('/api/office/copilot')) {
          ...
          req.on('end', async () => {
            try {
              const copilotHandler = (await import('./api/office/copilot.ts')).default
              const mockRes: any = {
                statusCode: 200,
                headers: {},
                setHeader(k: string, v: string) {
              ...
              await copilotHandler(mockReq, mockRes)
            } catch (err: any) {
              res.statusCode = 500
              res.setHeader('Content-Type', 'application/json')
              res.end(JSON.stringify({ error: err.message || 'Office copilot handler failed' }))
            }
          })
          return
        }
